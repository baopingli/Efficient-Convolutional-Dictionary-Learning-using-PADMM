    ...
    for file=image_frames{1}:image_frames{2}:image_frames{3}
        ...
        if (files(file).isdir == 0)
            ...
            try
                ...
                IMG = single(imread(img_name));%这里将图像转换为了单精度。
...
for i=1:length(I)
    switch(COLOR_TYPE)
        ...
        case 'gray'
            ...
            if(size(I{i},3)==3)
                I{i} = single(rgb2gray(double(I{i})/255.0));%执行的这里的函数
...
switch(CONTRAST_NORMALIZE)
    ...
    case 'local_cn'
        ...
        k = fspecial('gaussian',[13 13],3*1.591);%滤波算子，13是摸板尺寸，后面的是滤波器的标准值
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Square the images to the max dimension.将图像平方到最大维度
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%然后将cell转换成矩阵然后double一下作为了输入的变量
